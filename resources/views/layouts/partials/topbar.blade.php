{{-- Topbar --}}
<header class="sticky top-0 z-20 bg-gray-900/95 backdrop-blur-md border-b border-gray-700/50">
    <div class="flex items-center justify-between h-14 px-4 lg:px-6">
        {{-- Mobile menu button --}}
        <button onclick="toggleSidebar()" class="lg:hidden p-2 rounded-lg text-gray-300 hover:bg-gray-800 transition">
        ...
        {{-- Page title --}}
        <h2 class="text-base font-semibold text-white hidden lg:block">
            {{ $title ?? 'Dashboard' }}
        </h2>
        ...
        <div class="flex items-center space-x-3">
            {{-- Clock --}}
            <span class="text-xs text-gray-400 hidden sm:block" id="live-clock"></span>

            {{-- Logout --}}
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-sm text-gray-300 hover:bg-red-900/30 hover:text-red-400 transition">
