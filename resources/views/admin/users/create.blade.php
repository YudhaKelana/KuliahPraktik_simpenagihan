    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
            ...
            <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-4">@csrf
                ...
                <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Role</label><select name="role" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-sm dark:text-white"><option value="petugas_penagihan" {{ old('role') === 'petugas_penagihan' ? 'selected' : '' }}>Petugas Penagihan</option><option value="koordinator_penagihan" {{ old('role') === 'koordinator_penagihan' ? 'selected' : '' }}>Koordinator Penagihan</option><option value="administrator_sistem" {{ old('role') === 'administrator_sistem' ? 'selected' : '' }}>Administrator Sistem</option></select></div>
                <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Password <span class="text-gray-400 font-normal">(minimal 8 karakter)</span></label><input type="password" name="password" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-sm dark:text-white">@error('password')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror</div>
