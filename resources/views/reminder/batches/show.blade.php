<x-layouts.app :title="'Detail Batch #' . $batch->id">
    <div class="max-w-5xl mx-auto">
        <a href="{{ route('reminder.batches.index') }}" class="inline-flex items-center text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 mb-4">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 mb-6 shadow-sm">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{{ $batch->filter_description ?? '-' }}</p>
                </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm mb-4">
                <div><span class="text-gray-500 dark:text-gray-400">Total Item</span><p class="font-medium text-gray-900 dark:text-white">{{ $batch->total_items }}</p></div>
                <div><span class="text-gray-500 dark:text-gray-400">Terkirim</span><p class="font-medium text-emerald-600 dark:text-emerald-400">{{ $batch->sent_count }}</p></div>
                <div><span class="text-gray-500 dark:text-gray-400">Gagal</span><p class="font-medium text-red-600 dark:text-red-400">{{ $batch->failed_count }}</p></div>
                <div><span class="text-gray-500 dark:text-gray-400">Dilewati</span><p class="font-medium text-gray-500 dark:text-gray-400">{{ $batch->skipped_count }}</p></div>
            </div>
            {{-- Actions --}}
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                            ...
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-300">{{ $item->vehicle->taxpayer->name ?? '-' }}</td>
                            <td class="px-4 py-3 text-center text-gray-500 dark:text-gray-400 text-xs">{{ $item->rule->name ?? '-' }}</td>
                            <td class="px-4 py-3 text-center text-gray-500 dark:text-gray-400 text-xs">{{ $item->planned_send_at?->format('d/m H:i') ?? '-' }}</td>
