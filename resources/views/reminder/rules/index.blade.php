    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 dark:bg-gray-700/50 text-xs text-gray-500 dark:text-gray-400 uppercase"><tr><th class="px-4 py-3 text-left">Nama</th><th class="px-4 py-3 text-center">Hari</th><th class="px-4 py-3 text-center">Window</th><th class="px-4 py-3 text-center">Status</th><th class="px-4 py-3 text-center">Aksi</th></tr></thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse($rules as $rule)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                        <td class="px-4 py-3"><p class="font-medium text-gray-900 dark:text-white">{{ $rule->name }}</p><p class="text-xs text-gray-400 dark:text-gray-500">{{ $rule->template_code }}</p></td>
                        <td class="px-4 py-3 text-center font-bold text-blue-600 dark:text-blue-400">H-{{ $rule->days_before_due }}</td>
                        <td class="px-4 py-3 text-center text-gray-500 dark:text-gray-400 text-xs">{{ $rule->send_window_start }} — {{ $rule->send_window_end }}</td>
