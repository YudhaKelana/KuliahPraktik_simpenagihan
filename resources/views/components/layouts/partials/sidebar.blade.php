...
<aside id="sidebar" class="fixed inset-y-0 left-0 z-40 w-64 bg-gradient-to-b from-slate-800 to-slate-900 text-white transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col">
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        {{-- MONITORING PENAGIHAN - Semua role --}}
        <div class="pt-4">
        ...
        {{-- Kinerja Pegawai - Admin + Koordinator --}}
        @if(auth()->user()->hasRole(['administrator_sistem', 'koordinator_penagihan']))
        ...
        {{-- TOOLS - Koordinator + Petugas (non-admin import) --}}
        @if(!auth()->user()->isAdmin() && auth()->user()->hasRole(['koordinator_penagihan', 'petugas_penagihan']))
        <div class="pt-4">
    ...
    <div class="border-t border-slate-700/50 p-4">
        <div class="flex items-center space-x-3">
            ...
            <div class="flex-1 min-w-0">
                ...
                <p class="text-[10px] text-slate-400">{{ auth()->user()->getRoleLabel() }}</p>
            </div>
        </div>
    </div>
</aside>
